...
class ChessPlayer:
    def __init__(self, config: Config, pipes=None, play_config=None,
                 dummy=False):
        self.moves = []
        ...
        self.move_lookup = {chess.Move.from_uci(move): i for move, i in
                            zip(self.labels, range(self.labels_n))}
        if dummy:
            return
    ...
    def search_moves(self, env) -> (float, float):
        ...
        with ThreadPoolExecutor(
                max_workers=self.play_config.search_threads) as executor:
            for _ in range(self.play_config.simulation_num_per_move):
                futures.append(
                    executor.submit(self.search_my_move, env=env.copy(),
                                    is_root_node=True))
    ...
    def select_action_q_and_u(self, env, is_root_node) -> chess.Move:
        ...
        xx_ = np.sqrt(
            my_visitstats.sum_n + 1)  # sqrt of sum(N(s, b); for all b)
