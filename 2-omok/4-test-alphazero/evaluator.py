from utils import render_str, get_action, get_state_pt, get_action_eval
from agents import ZeroAgent, PUCTAgent, RandomAgent, HumanAgent
...
class Evaluator:
    def __init__(self, model_path_a, model_path_b):
        ...
        if model_path_a == 'puct':
            ...
            print('load player model:', model_path_a)
            self.player = ZeroAgent(STATE_SIZE, N_MCTS, IN_PLANES)
        ...
        if model_path_b == 'human':
            ...
            print('load enemy model:', model_path_b)
            self.enemy = ZeroAgent(STATE_SIZE, N_MCTS, IN_PLANES)
    ...
    def get_action(self, root_id, board, turn, enemy_turn):
        if turn != enemy_turn:
            ...
            action, action_index = get_action_eval(p, board)
        else:
            if self.model_path_b == 'random':
                pi = self.enemy.get_pi(root_id, board, turn, tau=1)
                action, action_index = get_action_eval(pi, board)
